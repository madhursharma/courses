...
def bond_cash_flows_monthly(maturity, principal=100, coupon_rate=0.03, coupons_per_year=4, ts_freq=12):
    """
    Returns the series of cash flows generated by a bond,
    indexed by the payment/coupon number
    ts_freq to space out the coupon payments
    """
    n_coupons = round(maturity*coupons_per_year)
    coupon_amt = principal*coupon_rate/coupons_per_year
    
    coupon_times = [x for x in range(0, (maturity*ts_freq +1), int(ts_freq/coupons_per_year))]
    coupons = np.repeat(coupon_amt, len(coupon_times))
    #TODO: Done change first coupon to 0
    coupons[0] = 0.0
    cash_flows = pd.Series(data=coupons, index=coupon_times)
    cash_flows.iloc[-1] += principal
    return cash_flows
...
def match_durations_mod (cf_t, cf_s, cf_l, discount_rate, coupons_per_year):
    """
    Returns the weight W in cf_s that, along with (1-W) in cf_l will have an effective
    duration that matches cf_t
    """
    d_t = macaulay_duration(cf_t, discount_rate/coupons_per_year)/coupons_per_year
    d_s = macaulay_duration(cf_s, discount_rate/coupons_per_year)/coupons_per_year
    d_l = macaulay_duration(cf_l, discount_rate/coupons_per_year)/coupons_per_year
    return (d_l - d_t)/(d_l - d_s)
